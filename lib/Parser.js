...
var grammar = function () {
  ...
  var raw = {
    ...
    equinox: /^(?:_count_days _direction )?(spring|summer|autumn|winter) (?:equinox|solstice)?(?: _count_days)?(?: in ([^\s]*))?/,
  ...
  raw.equinox = replace(raw.equinox, '')(/_count_days/g, raw._count_days)(/_direction/g, raw._direction)();
...
var Parser = function () {
  function Parser(fns) {
    ...
    this.fns = fns || ['_date', '_easter', '_islamic', '_hebrew', '_equinox', '_dateMonth', '_ruleDateIfThen', '_ruleYear', '_ruleDateDir', '_ruleBridge', '_ruleTime', '_ruleDuration', '_ruleModifier'];
    this.tokens = [];
  }
  ...
  _createClass(Parser, [{
    ...
    /**
     * check if rule was fully parsed
     * @return {boolean}
     */

  }, {
    key: 'fullyParsed',
    value: function fullyParsed() {
      return !this.setup.str;
    }
    ...
    value: function _equinox(o) {
      ...
      if (cap = grammar.equinox.exec(o.str)) {
        this._shorten(o, cap[0]);
        cap.shift();
        var count = toNumber(cap.shift());
        var direction = cap.shift();
        var res = {
          fn: 'equinox',
          season: cap.shift(),
          offset: toNumber(cap.shift()) || 0,
        ...
        if (direction && count) {
          res.offset = direction === 'before' ? -1 * count : count;
        }
        this.tokens.push(res);
        return true;
      }
