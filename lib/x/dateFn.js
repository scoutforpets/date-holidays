...
var DAYS = function () {
  var o = {};
  var i = 0;
  'sunday|monday|tuesday|wednesday|thursday|friday|saturday'.split('|').forEach(function (day) {
    o[day] = i++;
  });
  return o;
}();
...
function _equinox(d) {
  ...
  return function (year) {
    ...
    var str = zero(mbdate.year, 4) + '-' + zero(mbdate.month) + '-' + zero(Math.floor(mbdate.day)) + 'T' + zero(mbhour.hour) + ':' + zero(mbhour.minute) + ':' + zero(mbhour.second) + 'Z';
...
function _dateBridge(d, rules) {
  ...
  function F(year) {
    ...
    for (i in rules) {
      if (F !== rules[i].fn) {
        // prevent recursion for this function
        tmp = rules[i].fn(year);
        if (tmp && (tmp = tmp.start)) {
          tmp.month = tmp.getMonth() + 1;
          tmp.day = tmp.getDate();
          for (j in d.isDates) {
            if (tmp.month === d.isDates[j].month && tmp.day === d.isDates[j].day) {
  ...
  return function (year) {
    ...
    if (d.other) {
      ...
      dArr = tmp.map(function (t) {
        ...
        if (d.other.fn === 'islamic' || d.other.fn === 'jewish') {
          date = t.end;
        }
